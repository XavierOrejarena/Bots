...
def teleported():
	...
	if get_zone_name(get_character_data()['region']) == 'Diebesstadt':
		stop_bot()
		Timer(1,move_to,[9113,876,0]).start()
		Timer(4,talkToBandit).start()
...
def easyPick():
	...
	if drops:
		for dropID in drops:
			if 'inmortal' in drops[dropID]['name'].lower():
				inject_joymax(0x7074, b'\x01\x02\x01' + struct.pack('I', dropID), False)
				Timer(0.3,easyPick).start()
				return
		for dropID in drops:
			if drops[dropID]['name'] == 'Magic POP Card':
				inject_joymax(0x7074, b'\x01\x02\x01' + struct.pack('I', dropID), False)
				Timer(0.3,easyPick).start()
				return
		for dropID in drops:
			if drops[dropID]['name'] == 'Reverse Reverse Return Scroll':
				inject_joymax(0x7074, b'\x01\x02\x01' + struct.pack('I', dropID), False)
				Timer(0.3,easyPick).start()
				return
		for dropID in drops:
			if drops[dropID]['name'] == 'Global chatting':
				inject_joymax(0x7074, b'\x01\x02\x01' + struct.pack('I', dropID), False)
				Timer(0.3,easyPick).start()
				return
		for dropID in drops:
			if drops[dropID]['name'] == 'Gold':
				inject_joymax(0x7074, b'\x01\x02\x01' + struct.pack('I', dropID), False)
				Timer(0.3,easyPick).start()
				return
...
def handle_silkroad(opcode,data):
	...
	if opcode == 0x706D:
		...
		for slot, item in enumerate(get_inventory()['items']):
			if item:
				if item['name'] == 'Energy of Life 4th Degree':
					Packet = bytearray()
					Packet.append(slot)
					Packet.append(0xEC)
					Packet.append(0x76)
		...
		for slot, item in enumerate(get_inventory()['items']):
			if item:
				if item['name'] == 'Energy of Life 2nd Degree':
					Packet = bytearray()
					Packet.append(slot)
					Packet.append(0xEC)
					Packet.append(0x76)
...
def quitarDressLucky():
	...
	for slot, item in items:
		if item:
			if item['name'] == 'Chaos Lord Dress (M)':
				log(item['name'])
	...
	for slot, item in items:
		if item:
			if item['name'] == 'Samurai Accessory (M)':
				...
				Timer(0.5,quitarDressLucky).start()
				return
	if get_inventory()['items'][13]['name'] == 'Bolt':
		start_bot()
	return
...
btn5 = QtBind.createButton(gui,'cancelReturnScroll','Cancelar Scroll',100,185)
...
btn8 = QtBind.createButton(gui,'DC','DC',100,210)
